...
namespace BoggleService.Controllers
{
    ...
    public class BoggleController : ApiController
    {
        ...
        public string PostRegister([FromBody]string name)
        {
            if (name == "stall")
            {
                Thread.Sleep(5000);
        ...
        /// </summary>
        [Route("BoggleService/games")]
        public void PutCancelGame([FromBody]string user)
        {
            lock (sync)
            {
        ...
        public int PutPlayWord([FromUri] string gameID, PlayWordInput input)
        {
        ...
        public Object GetGameStatus([FromUri]string gameID, [FromUri]bool brief)
        {
